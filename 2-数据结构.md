...
inline int qRange(int x,int y)  //从x到y路径上权值和
{
    ...
    while(top[x]!=top[y]) //当两个点不在同一条链上
    {
        ...
        res=0;
        query(1,1,n,dfn[top[x]],dfn[x]);//ans加上x点到x所在链顶端 这一段区间的点权和
        ans+=res;
        ans%=mod;//按题意取模
    ...
    res=0;
    query(1,1,n,dfn[x],dfn[y]);//这时再加上此时两个点的区间和即可
    ans+=res;
    return ans%mod;
}
...
{
    res=0;
    query(1,1,n,dfn[x],dfn[x]+sz[x]-1);//子树区间右端点为dfn[x]+sz[x]-1
    return res;
}
...
ll query(int v,int u,int l,int r,int ql,int qr)
{
    if(!u && !v) return 0;
