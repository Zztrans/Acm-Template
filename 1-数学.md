...
void getprime() {
    notprime[0]=notpraaaime[1]=1;phi[1]=mu[1]=1;
    cnt=0;
...
auto w = getOmega<1 << 20>();//大部分operator可以不抄
...
void solve(int l,int r) {//f_i = \sum_ j=0 ^{i-1} f_j*g_{i-1-j}
    if(l==r) return;
    int mid=l+r>>1;
    solve(l,mid);
    int len=mid-l+1;
    Poly tmpf(len),tmpg(2*len);
    for(int i=l;i<=mid;++i) tmpf[i-l]=f[i];
    for(int i=0;i<2*len;++i) tmpg[i]=g[i];
    tmpf=tmpf*tmpg;
    for(int i=mid+1;i<=r;++i) f[i]=(f[i]+tmpf[i-mid-1+len-1])%mod;
    solve(mid+1,r);
}
...
ll b[N][60]; int pos[N][60]; //每个右端点继承前一个再插入
void insert(ll x,int r){
    ...
    for(int i=60;i>=0;i--) {
        if(x>>i&1) {
            ...
            if(pos[r][i]<p) swap(pos[r][i],p),swap(x,b[r][i]);
            x^=b[r][i];
        }
    }
}
...
struct Linebasis{
    ...
    Linebasis operator +(const Linebasis & r) const{
        Linebasis res=r;
        for(int i=0;i<=62;++i) if(p[i]) res.insert(p[i]);
        return res;
    }
    ll querymax(ll x=0){
        ll res=x;
    ...
    void rebuild(){
        ...
        cnt = 0;
        for(int i = 0; i <= 62; ++i) if(p[i]) b[cnt++] = p[i];
    }
    ll kthmax(ll k){
        if(flag) k--;
        ll res = 0;
        ...
        for(int i=62;i>=0;--i) if(k>>i&1) res^=b[i];
        return res;
    }
}sol;
```
