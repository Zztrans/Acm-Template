...
```c++
bool notprime[maxn];
int prime[maxn/10],mu[maxn],phi[maxn],cnt;
void getprime()
{
...
ll det(int n){
	...
	for(int i=1;i<=n;++i){
	    int p0=i;
	    for(int j=i+1;j<=n;++j) if(abs(a[p0][i])<abs(a[j][i]))p0=j;
	    if(a[p0][i]==0) return 0;
	    if(p0!=i){ans=mod-ans;for(int j=1;j<=n;++j)swap(a[i][j],a[p0][j]);}
	    v=inv(a[i][i]),ans=ans*a[i][i]%mod;
	    for(int j=1;j<=n;++j){
	        if(j==i)continue;
			tmp=v*a[j][i]%mod;
	        for(int k=i;k<=n;++k) a[j][k]=(a[j][k]-tmp*a[i][k])%mod;//possibly -
	    }
	}
	return (ans+mod)%mod;
}
